...
> 각 구성 요소는 검증된 기술이지만,  
> **이탈 감지 → NPC 개입 → 퀘스트/상품 연결을 하나로 연결한 구현체는 공개된 자료 기준으로 찾지 못했습니다.**
...
> ⚠️ **본 결과는 합성 데이터 기반 시뮬레이션입니다.**  
> 이탈 패턴 정의와 개입 효과 가정값이 본인 설계에 의존하므로, 절대 수치보다 **파이프라인 동작 검증**을 목적으로 합니다.  
> 실제 게임 로그 적용 시 수치는 달라질 수 있으며, 파일럿 단계에서 Blade & Soul IEEE CIG 2017 데이터 등으로 재검증 예정입니다.
...
본 파이프라인 도입 시 각 조직의 역할 분담:

| 영역 | 담당 | 주요 역할 |
|---|---|---|
| **PM (본 프로젝트 설계 범위)** | 사업 PM | KPI 정의, 파이프라인 기획, A/B 설계, 비용-효과 분석, 로드맵 수립 |
| **게임 기획팀** | 게임 디자이너 | NPC 세계관 일관성 검토, 대사 톤·매너 가이드, 퀘스트 밸런스 조율 |
| **라이브 운영팀** | 라이브 Ops | 개입 빈도 정책, 유저 컴플레인 대응, 이벤트 캘린더와 충돌 조율 |

> AI가 생성하는 NPC 대사와 상품 제안은 반드시 게임 기획팀의 세계관 가이드를 준수해야 합니다.  
> 라이브 운영팀은 자동 개입 정책의 on/off 및 예외 처리를 담당합니다.
...
| 파일럿 | 4~6개월 | 실제 게임 로그 연동, AI 맞춤 상품 제안, BM 전환 실험 | D7 리텐션 +5%p, CVR +3%p / 합성 데이터 룰의 실제 데이터(Blade & Soul IEEE CIG 2017) 재검증 |
...
> **AI 활용 공개:** 본 PoC는 Claude Sonnet 4.6을 코딩 보조 도구로 활용하여 구현했습니다.  
> 문제 정의, 시장 조사, 이탈 시나리오 설계, KPI 정의, 피처 설계는 본인이 직접 수행했습니다.
